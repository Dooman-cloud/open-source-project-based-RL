...
RISKDOC는 **Streamlit 단일 프로세스 모놀리스**이다. REST API 서버·메시지 큐·관계형 DB는 사용하지 않으며, 프레젠테이션·비즈니스 로직·파일 기반 데이터 접근이 하나의 Python 프로세스에서 실행된다.
...
| 계층               | 디렉터리                                               | 책임                                   |
| ---------------- | -------------------------------------------------- | ------------------------------------ |
| **Presentation** | `frontend/app.py`, `frontend/components/charts.py` | UI, 위젯, `@st.cache_data`, 챗봇, 장마감 라벨 |
...
- 혁신 분포: Student-t (`dist="t"`)
- `arch` 피팅 시 수익률 ×100, σ 결과 ÷100
- 1-step ahead `forecast_volatility` → 당일 VaR·ES에 사용
...
**설계 결정**: GARCH/VaR은 사용자 선택 **분석 기간** 구간으로 슬라이스하되, SMA60·BB 등은 **5년 전체 데이터**로 계산 후 동일 구간만 잘라 사용 — 워밍업 구간 부족 방지.
...
4. 2:1 컬럼 — 좌: 차트 3종 / 우: TOP3 랭킹 + 챗봇(600px)
...
`GARCHResult` dataclass로 조건부 변동성 시계열·파라미터·AIC/BIC·예측 σ 반환.
...
- RISKDOC 브랜드 CSS (그라데이션·Inter/JetBrains Mono)
...
| fetch (1종목)          | <1s    | 1~5s           |
| GARCH 피팅             | —      | 0.5~3s         |
| load_analysis 전체     | <1s    | 2~5s           |
| get_ranking (9종목 추가) | <1s    | 15~40s         |
...
| **꼬리 리스크** | Student-t로 정규 대비 보수적                            |
| **비대칭성**   | GJR로 하락 충격 반영                                   |
| **백테스트**   | 본 프로젝트 범위 외 (향후 과제)                             |
...
| ID   | 심각도 | 설명                   |
| ---- | --- | -------------------- |
| D-01 | 중   | Fallback 데이터 UI 미고지  |
| D-02 | 중   | 랭킹 실패 종목 silent skip |
...
| 도구                        | 버전/모델      | 주 용도           |
| ------------------------- | ---------- | -------------- |
| **Cursor IDE**            | Composer   | 문서 초안 생성성      |
| **Anthropic Claude Code** | Sonnet 4.6 | 코드 생성·리팩터링·디버깅 |
| **GitHub Copilot**        | —          | 자동완성 (부분 사용)   |
...
| 단계               | AI 활용 내용                                                                                            | 인간 검증                |
| ---------------- | --------------------------------------------------------------------------------------------------- | -------------------- |
| **데이터 수집**       | Anthropic Claude Code로 `TICKERS` 초안 생성                                                              | import 경로·패키지 버전 확인  |
...
| **문서**           | Cursor로 README, DEVELOPER_GUIDE, USER_GUIDE, 최종보고서 초안 작성                                            | 코드와 불일치 부분 교정정       |
| **버그 수정**        | 투자금 캐시 키 누락, 신뢰수준 슬라이더, 5y 지표 슬라이스 등 Github Copilot/Anthropic Claude Code 디버깅 세션 다수                 | 수동 테스트               |


**준수 사항**: AI 생성 코드는 반드시 실행·리뷰 후 merge. API 키·`.env`는 AI 채팅에 업로드하지 않음.
